myApp.controller("buyer-CategoryCtrl", function ($scope, $stateParams, Navigation, $state, ionicToast, $ionicActionSheet, $cordovaCamera, $cordovaFileTransfer, $cordovaImagePicker) {
  ...
  /**if user selects Accessories tab */
  $scope.selectedAccessory = function () {
    if ($scope.user.isAccessories) {
      console.log("isAccessories", $scope.user.isAccessories);
    }
    Navigation.commonAPICall("User/save",
      $scope.user,
      function (data) {
        if (data.data.value) {
          console.log("saved");
   ...
   * */
  $scope.goTo = function () {
    if ($scope.user.category.length != 0 || $scope.user.isAccessories) {
      if (!$scope.user.isAccessories) {
        _.remove($scope.user.category, function (cat) {
